...
/// so an exported observation's <c>sosa:hasFeatureOfInterest</c> reflects where the
/// measurement was actually taken. Placement periods are half-open
/// <c>[ValidFrom, ValidTo)</c>; an instant before a sensor's first placement (or for an
/// unknown sensor) resolves to null and the feature is omitted. Mirrors Public.Api's
/// resolver — the export streams the ieq database and cannot share its types.
/// </summary>
public sealed class FeatureOfInterestResolver
{
    public static readonly FeatureOfInterestResolver Empty = new([]);
