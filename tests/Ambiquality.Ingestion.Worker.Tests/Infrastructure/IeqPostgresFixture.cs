...
namespace Ambiquality.Ingestion.Worker.Tests.Infrastructure;

/// <summary>
/// A TimescaleDB container with the <c>ieq</c> schema migrated (the migration runs
/// <c>create_hypertable</c>, so the timescale image is required). The worker only
/// touches <c>ieq</c>, so unlike the API tests no evidence schema is provisioned.
/// </summary>
